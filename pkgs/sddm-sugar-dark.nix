{ stdenv, fetchFromGitHub, pkgs, themeConfig ? {} }:
  stdenv.mkDerivation {
  ...
  # [Interface Behavior]
    ...
    installPhase = ''
      ...
      '';
      #cp $theme $out/share/sddm/themes/sddm-sugar-dark/theme.conf
    preFixup = ''
    cd $out/share/sddm/themes/sddm-sugar-dark/ 
    mv theme.conf theme.conf.old
    cp $theme ./theme.conf
    '';
    src = fetchFromGitHub {
      owner = "MarianArlt";
